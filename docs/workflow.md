...
4. **Normalize Values & Types**
   ...
   - Після застосування синонімів `NormalizationService` синхронізує схеми з реєстром `CanonicalSchemaRegistry`: проставляє `canonical_schema_id`, оновлює `SchemaColumn.data_type` і блокувальні прапори `required`/`allow_null` для наступних фаз.
