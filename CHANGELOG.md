...
## [0.3.0] - 2025-12-04
### Added
- Job lifecycle tracking: `JobStateMachine` emits transitions `PENDING → MATERIALIZING → VALIDATING → DONE/FAILED`, CLI now accepts `--job-id`, and SQLite gained `job_status`/`job_events` tables + `storage.fetch_job_status` accessor (documented in `docs/architecture/job_lifecycle.md`).
