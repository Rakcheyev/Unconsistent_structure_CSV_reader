...
- Запущено єдиний Job State Machine: `uscsv materialize` приймає `--job-id` (авто-генерується), `JobStateMachine` фіксує переходи `PENDING → ANALYZING → MAPPING → MATERIALIZING → VALIDATING → DONE/FAILED/CANCELLED`, а SQLite отримав таблиці `job_status` + `job_events` з доступом через `storage.fetch_job_status` (див. `docs/architecture/job_lifecycle.md`).
- Конфігураційний шар тепер описується `ConfigDocument`: JSON перевіряє всі обов'язкові поля/шляхи, нормалізує `error_policy`, і в разі проблем викидає `BackendError` з кодом (`CONFIG_ERROR`, `IO_ERROR`, `STATE_ERROR`), який CLI відображає для агентів.
- Файлова пісочниця (`common.sandbox.Sandbox`) огортає всі CLI-шляхи, розв'язує їх відносно робочого каталогу job'а, блокує `..`/абсолютні втечі за межі root та, за потреби, пропускає лише явно дозволені директорії через allowlist; усі порушення піднімають `BackendError` із кодом стану.
